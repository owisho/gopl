...
func lissajous(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		fmt.Fprintf(w, "parse data error = %v", err)
		return
	}
	rcycles, err := strconv.Atoi(r.FormValue("cycles"))
	if err != nil {
		fmt.Fprintf(w, "get cycles param error=%v", err)
		return
	}
	const (
		cycles  = 5
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < float64(rcycles)*2*math.Pi; t += res {
			x := math.Sin(t)
